 ...
 * @Last modified time: 2017-05-26T13:46:42-05:00
...
export class SentimentChartComponent implements OnInit, OnChanges {
  ...
  constructor(private _appComm: AppCommService) {
    _appComm.appComm$.subscribe((event: AppCommEvent) => {
      ...
      //
      if (event.type === AppCommService.typeEnum.nlu && event.subType === AppCommService.subTypeEnum.nlu.sentiment) {
      ...
      //
      if (event.type === AppCommService.typeEnum.nlu && event.subType === AppCommService.subTypeEnum.nlu.emotion) {
